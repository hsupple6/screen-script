...
const Home: React.FC<HomeProps> = ({ title = 'Welcome to Screen Script' }) => {
  ...
  // Function to handle model command
  const handleModelCommand = async (model: string, size: string, args: string[] = []) => {
    try {
      const response = await fetch('http://localhost:5421/api/command/model', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ model, size, args }),
      });
      ...
      console.error('Error sending model command:', error);
    }
  };
  ...
  return (
    <div className="home-container">
      ...
      <div className="home-head-opacity"></div>
      <div className='home-head'>
        <img src={GalLogo} alt="Gal Logo" 
        style = {{zIndex: "3"}}/>
        <div className = "welcome-message" style = {{position: "absolute", top: "50%", padding: "10px", paddingLeft: "100px"}}>
          Welcome!
        </div>
